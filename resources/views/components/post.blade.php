<div id="{{ $post->id ?? ""}}" {{ $attributes->merge(["class" => "post transparent-border flex flex-col w-4/5 p-2 mb-3 ".$classoptions]) }}>
    ...
    <div class="post__activity flex justify-start">
        <p>
            <i style="color: #2980B9;" class="fas fa-thumbs-up"></i>
            <h6 class="mx-1 post__activity__likes">{{ $post->likes->count() ?? ""}}</h6>
        </p>
        <p>
            <i style="color: #2980B9;" class="far fa-comments"></i>
            <h6 class="mx-1 post__activity__comments">{{ $post->comments->count() ?? ""}}</h6>
        </p>
    </div>

    <div class="post__buttons flex justify-center mx-1 border-b-2 border-gray-200 pb-5">
        <button class="bg-gray-200 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-l">
            <i style="color: #2980B9; font-size: 16px" class="fas fa-thumbs-up"></i>
        </button>

        <button class="bg-gray-200 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-r">
            <i style="color: #2980B9; font-size: 16px" class="far fa-comments"></i>
        </button>
    </div>

    @if($post != null)
        <x-comments :post="$post" :comments="$post->comments()->latest()->take(5)->with('author')->get()->sortBy('created_at')" />
    @else
        <x-comments />
    @endif
</div>
