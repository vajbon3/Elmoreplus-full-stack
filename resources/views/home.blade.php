<?php
?>
<x-main>
    <x-navbar :notifications="$notifications"/>
    <div class="grid grid-cols-5 gap-4 pt-36">
        <div class="col-span-1 flex flex-col items-center">
            <x-user-widget :user="Auth::user()"/>
        </div>
        <div class="col-span-3 flex flex-col justify-center items-center">
            <x-form id="createPost" method="POST" action="/api/posts" class="create-post w-2/3 m-2 h-auto">
                <h3>Create a post</h3>
                <x-form-textarea name="body" placeholder="tell us about your day" rows="3" cols="70"/>
                <div class="flex justify-around ">
                    <x-form-submit>
                        Post
                    </x-form-submit>
                </div>
            </x-form>
            <div id="posts" class="w-full flex flex-col items-center ">
                <div id="post_template" class="hidden transparent-border flex flex-col w-4/5 p-2 mb-3">
                    <div class="post__author flex flex-col justify-center items-start">
                        <div class="flex justify-center items-center">
                            <img style="width:40px" src="data:image/png;base64," alt="">
                            <div class="flex flex-col">
                                <a href="#"><h5 class="author_name"></h5></a>
                                <h6 style="font-size: 12px">just now</h6>
                            </div>
                        </div>
                    </div>
                    <div class="post__content flex flex-col ">
                        <div class="post__content__body">
                            <p class="post_body"></p>
                        </div>
                    </div>
                    <div class="post__activity flex justify-start">
                        <p>
                            <i style="color: #2980B9;" class="fas fa-thumbs-up"></i>
                        <h6 class="mx-1 post__activity__likes">0</h6>
                        </p>
                        <p>
                            <i style="color: #2980B9;" class="far fa-comments"></i>
                        <h6 class="mx-1 post__activity__comments">0</h6>
                        </p>
                    </div>

                    <div class="post__buttons flex justify-center mx-1 border-b-2 border-gray-200 pb-5">
                        <button class="bg-gray-200 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-l">
                            <i style="color: #2980B9; font-size: 16px" class="fas fa-thumbs-up"></i>
                        </button>

                        <button class="bg-gray-200 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-r">
                            <i style="color: #2980B9; font-size: 16px" class="far fa-comments"></i>
                        </button>
                    </div>
                </div>

                @foreach($posts as $post)
                    <x-post :post="$post" />
                @endforeach
            </div>
        </div>
    </div>
    <script src="{{ mix("js/home.js") }}" defer></script>
</x-main>
