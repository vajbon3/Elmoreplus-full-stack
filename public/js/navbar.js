...
positioned = false; // notifications container

var container = document.querySelector("#notifications"); // notification button

var notificationButton = document.querySelector("#notification-button");

function toggle(e) {
  container.classList.contains("invisible") ? container.classList.remove("invisible") : container.classList.add("invisible");
}
...
function positionPopupOnPage(evt) {
  ...
  var intDistanceScrolledUp = document.body.scrollTop; // distance the page has been scrolled up from view port top

  var intPopupOffsetTop = intDistanceScrolledUp + intCoordY; // add the two for total distance from click point y to top of page
  ...
  intVPW = VPWH[0];
  intVPH = VPWH[1];
  container.style.display = "block"; // if not display: block, .offsetWidth & .offsetHeight === 0

  container.style.zIndex = '10100';

  if (intCoordX > intVPW / 2) {
    intPopupOffsetLeft -= container.offsetWidth;
  } // if x is in the right half of the viewport, pull popup left by its width


  if (intCoordY > intVPH / 2) {
    intPopupOffsetTop -= container.offsetHeight;
